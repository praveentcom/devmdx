...
function processLists(html: string): string {
  ...
  for (const line of lines) {
    ...
    if (taskUncheckedMatch) {
      ...
      listItems.push(
        `<li class="mb-2 flex items-center gap-2"><input type="checkbox" disabled class="rounded border-border text-primary focus:ring-primary/50" /> ${taskUncheckedMatch[1]}</li>`,
      );
      ...
      listItems.push(
        `<li class="mb-2 flex items-center gap-2"><input type="checkbox" checked disabled class="rounded border-border text-primary focus:ring-primary/50" /> ${taskCheckedMatch[1]}</li>`,
      );
