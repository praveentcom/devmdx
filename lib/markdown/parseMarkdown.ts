import processLists from "@workspace/ui/lib/markdown/processLists";
...
function parseMarkdown(content: string): string {
  ...
  processedContent = processedContent
    ...
    .replace(
      /\[([^\]]+)\]\(([^)]+)\)/g,
      '<a href="$2" target="_blank" rel="noopener noreferrer" class="text-primary hover:underline font-medium inline-flex items-center gap-1">$1</a>',
    )
