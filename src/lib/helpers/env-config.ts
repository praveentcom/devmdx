 ...
 */
export function parseHostnames(envVar?: string, fallback: string[] = []): string[] {
 ...
 */
export function generateCSP(strictMode: boolean, allowedHostnames: string[] = []): string {
  if (strictMode) {
  ...
  const hostnameList = allowedHostnames.length > 0 
