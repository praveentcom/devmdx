...
export function parseMarkdown(content: string): string {
  ...
  processedContent = processedContent
    ...
    .replace(
      /^\- \[ \] (.*$)/gim,
      '<li class="ml-2 mb-0 text-sm leading-relaxed flex items-center gap-1.5"><input type="checkbox" disabled class="rounded border-border/75 text-primary focus:ring-primary/50" /> $1</li>'
    )
    .replace(
      /^\- \[x\] (.*$)/gim,
      '<li class="ml-2 mb-0 text-sm leading-relaxed flex items-center gap-1.5"><input type="checkbox" checked disabled class="rounded border-border/75 text-primary focus:ring-primary/50" /> $1</li>'
    )
    .replace(
      /^\- (.*$)/gim,
